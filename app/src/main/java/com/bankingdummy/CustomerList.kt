...
class CustomerList : AppCompatActivity(), CustomerItemClicked {
    ...
    private lateinit var helper: MyDB
    ...
    fun updateLayout() {
        ...
        if (firstperson != -1 && secondperson != -1) {
            val inflater: LayoutInflater = LayoutInflater.from(this)
            val subView: View = inflater.inflate(R.layout.transaction, null)
            ...
            //performing positive action
            builder.setPositiveButton("Approve", OnClickListener { dialog, which ->
                run {
